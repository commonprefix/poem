...
\begin{lemma}[Confirmation Separation]
...
\begin{proof}
  ...
  $[{:}{-k}] \rlhd C \neq [{:}{-k}] \lhd C$. Since these two subchains
  differ, there must be a block $B$ such that $B \in [{:}{-k}] \rlhd C$
  but $B \not\in [{:}{-k}] \lhd C$. Note that the other direction is impossible
  since, for any suffix $j \in \mathbb{N}$,
  $\rwork(C[{:}{-j}]) \leq \work(C[{:}{-j}])$,
  and, in particular, any $k$-work suffix of the real-work chain
  must necessarily contain at least the same blocks as the $k$-work suffix
  of the discrete-work chain, because the real work is at most
  the discrete work. % TODO: Add figure
  Let $B$ be the earliest such
  block. Therefore, $B = [-k] \rlhd C \neq [-k] \lhd C$.
  We conclude that there exists a \emph{subchain} $C^*$
  with $\work(C^*) \geq k(\kappa) > \rwork(C^*)$, namely
  $C^* = [{:}{-k}] \rlhd C$, noting that $B = C^*[0]$.
  Define $\borderline$ the bad event that there exists some subchain $C^*$
  in the execution such that $\work(C^*) \geq k(\kappa) > \rwork(C^*)$.

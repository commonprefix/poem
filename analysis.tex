...
where
$\rwork(C[{:}{i - 1}]) < \alpha \leq \rwork(C[{:}{i}])$.
...
\begin{definition}[\poem Typical Execution]\label{def:typicality}
  ...
  \begin{itemize}
    ...
    \item It is causal.
    \item It has hash separation. % TODO: this is undefined, but it looks like it's not needed due to the lemma that follows (Hash Seperation proven from causality); if removed, please also remove from Typicality Theorem proof
  \end{itemize}
\end{definition}
...
\begin{lemma}[Hash Separation]\label{lem:hash-separation}
  ...
  $\rwork(C_1) < \rwork(C_2)$, then $\work(C_1) < \work(C_2)$.
...
\begin{lemma}[Entropic Chain Growth Lemma]\label{lem:chain-growth}
  Suppose that at round $r_1$ an honest party has a chain which has real work $\rw$.
  Then, by round $r_2 \geq r_1$, every honest party adopts a chain which has real work at least
  $\rw + \sum_{r = r_1}^{r_2 - 1}{X_r}$.
\end{lemma}
\begin{proof}
  By induction on $r_2$. For the inductive base ($r_2 = r_1$), observe that
  if at round $r_1$ an honest party has a chain $C$ of work $\rw$, then
  ...
  greater or equal real work. % TODO: this is approximate work; invoke Hash Separation here

  For the inductive step, note that by the inductive hypothesis,
  every honest party has received a chain of real work at least $\rw' = \rw + \sum_{r = r_1}^{r_2 - 2}{X_r}$
  ...
  with a chain of real work at least $\rw' + X_{r_2 - 1}$ and broadcasted it to the network.
  At round $r_2$, every honest party receives the chain and adopts a chain
  of real work at least $\rw' + X_{r_2 - 1} = \rw + \sum_{r = r_1}^{r_2 - 1}{X_r}$. \Qed
\end{proof}
...
\begin{proof}
  ...
  In both cases, it follows that $[{:}{-k}] \rlhd C_1 \sim [{:}{-k}] \lhd C_2$.

...
$\work(C[{:}{i - 1}]) \leq \alpha < \work(C[{:}{i}])$.
If $\alpha$ is negative, then $[\alpha] \lhd C$ is defined as
the $i^\text{th}$ block of $C$ such that
$\work(C[{i - 1}{:}]) \leq -\alpha < \work(C[{i}{:}])$.
We write $[{\alpha}{:}{\beta}] \lhd C$ to denote
$C[{[\alpha] \lhd C}{:}{[\beta] \lhd C}]$.
The notation $[{\alpha}{:}] \lhd C$ means $C[{[\alpha] \lhd C}{:}]$,
and similarly $[{:}\beta] \lhd C$ means $C[{:}{[\beta] \lhd C}]$.
Given a chain $C$, we write $C.x$ to denote the ledger of $C$,
namely the sequence of transactions ${\big\lVert}_{B \in C} B.x$,
where $B.x$ denotes the sequence of transactions included in block $B$.

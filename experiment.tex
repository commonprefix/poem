...
In each simulated execution, we fix the block production rate $g$, the adversarial ratio $\beta$,
...
Having simulated the honest and adversary executions, we can now determine the latency of the system (whether this system is Bitcoin or PoEM).
...
simulate multiple such executions and record the minimum safe confirmation
...
To acquire $f$, we calculate the honest cumulative work increase rate for each execution and take their average.
Namely, for each execution, we take the last cumulative work increase that happened during its lifetime, and divide it by the time at which it occurred.
...
and for PoEM, also various bias parameters $\gamma$. The value of $g$ -- and potentially $\gamma$ -- that minimizes the latency of the system is
the optimal system parameterization for a given adversarial ratio $\beta$.
...
Following the above methodology, we obtain the results illustrated in Figures~\ref{fig:bitcoin_vs_poem}.
